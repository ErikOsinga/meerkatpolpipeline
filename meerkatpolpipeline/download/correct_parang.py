...
def create_logger():
    ...
    cfmt = logging.Formatter(('%(name)s - %(asctime)s %(levelname)s - %(message)s'))  # noqa: UP034
...
def add_column(table, col_name, like_col="DATA", like_type=None):
    ...
    if col_name not in table.colnames():
        ...
        desc[str('name')] = str(col_name)  # noqa: UP018
        desc[str('comment')] = str(desc['comment'].replace(" ", "_"))  # got this from Cyril, not sure why  # noqa: UP018
        dminfo = table.getdminfo(like_col)
        dminfo[str("NAME")] =  "{}-{}".format(dminfo["NAME"], col_name)  # noqa: UP018

        # if a different type is specified, insert that
        if like_type:
            desc[str('valueType')] = like_type  # noqa: UP018
...
with tbl(args.ms, ack=False) as t:
    with taql("select * from $t where FIELD_ID=={}".format(args.field)) as tt:  # noqa: UP032
        def __to_datetimestr(t):
            dt = datetime.datetime.utcfromtimestamp(quantity("{}s".format(t)).to_unix_time())  # noqa: UP032
            return dt.strftime("%Y/%m/%d %H:%M:%S")
        dbtime = tt.getcol("TIME_CENTROID")
        start_time_Z = __to_datetimestr(dbtime.min())
        end_time_Z = __to_datetimestr(dbtime.max())
        log.info("Observation spans '{}' and '{}' UTC".format(  # noqa: UP032
...
if not args.noparang:
    log.info("Computing PA in {} steps of {} mins each".format(nstep, args.stepsize))  # noqa: UP032
...
if args.ephem:
    ...
    if not fieldEphem:
        raise RuntimeError("Body {} not defined by PyEphem".format(args.ephem))  # noqa: UP032
    log.info("Overriding stored ephemeris in database '{}' field '{}' by special PyEphem body '{}'".format(  # noqa: UP032
    ...
    rahms = "{0:.0f}:{1:.0f}:{2:.5f}".format(*skypos.ra.hms)  # noqa: UP030
    decdms = "{0:.0f}:{1:.0f}:{2:.5f}".format(skypos.dec.dms[0], abs(skypos.dec.dms[1]), abs(skypos.dec.dms[2]))  # noqa: UP030, UP032
    fieldEphem = ephem.readdb(",f|J,{},{},0.0".format(rahms, decdms))  # noqa: UP032
    log.info("Using coordinates of field '{}' for body: J2000, {}, {}".format(fieldnames[args.field],  # noqa: UP032
...
if not args.noparang:
    ...
    if args.plot:
        ...
        for axl, axd in zip(["Az", "El", "RA", "DEC", "ParAng"],
            ...
            ax.set_ylabel("{} [deg]".format(axl))  # noqa: UP032
            if axl == "ParAng":
    ...
    with tbl(args.ms+"::FEED", ack=False) as t:
        with taql("select * from $t where SPECTRAL_WINDOW_ID=={}".format(spwsel)) as tt:  # noqa: UP032
            ...
            if args.fa is not None:
                ...
                log.info("Overriding F Jones angle to {0:.3f} for all antennae".format(float(args.fa)))  # noqa: UP030, UP032
            else:
                log.info("Applying the following feed angle offsets to parallactic angles:")
                for ai, an in enumerate(anames):
                    log.info("\t {0:s}: {1:.3f} degrees".format(an, np.rad2deg(receptor_angles.get(ai, 0.0))))  # noqa: UP030, UP032
...
with tbl(args.ms+"::SPECTRAL_WINDOW", ack=False) as t:
    ...
    log.info("Will apply to SPW {0:d} ({3:d} channels): {1:.2f} to {2:.2f} MHz".format(  # noqa: UP030, UP032
        spwsel, chan_freqs.min()*1e-6, chan_freqs.max()*1e-6, nchan))
list_apply = []
if abs(args.crossphase) > 1.0e-6:
    log.info("Applying crosshand phase matrix (X) with {0:.3f} degrees".format(args.crossphase))  # noqa: UP030, UP032
...
for j in list_apply:
    log.info("\t{}".format(j))  # noqa: UP032
if args.invertpa:
    log.warning("Note: Applying corrupting P+F Jones, instead of correction per user request")

if not args.sim:
    log.info("Storing corrected data into '{}'".format(args.storecolumn))  # noqa: UP032
    ...
    with tbl(args.ms, ack=False, readonly=False) as t:
        ...
        with taql("select * from $t where FIELD_ID=={} and DATA_DESC_ID=={}".format(args.field, args.ddid)) as tt:  # noqa: UP032
            nrow = tt.nrows()
            ...
            for ci in range(nchunk):
                ...
                def __casa_to_unixtime(t):
                    dt = quantity("{}s".format(t)).to_unix_time()  # noqa: UP032
                    return dt
                ...
                log.info("\tCorrected chunk {}/{}".format(ci+1, nchunk))  # noqa: UP032
                nrowsput += crow
